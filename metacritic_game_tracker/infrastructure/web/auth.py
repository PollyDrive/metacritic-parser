...
def require_operator(credentials: HTTPBasicCredentials = Depends(_security)) -> str:
    user_ok = secrets.compare_digest(credentials.username, os.environ["MONITORING_USERNAME"].strip())
    pass_ok = secrets.compare_digest(credentials.password, os.environ["MONITORING_PASSWORD"].strip())
