...
def _game(id_, title, platform):
    ...
    game.platform_scores = [MagicMock(platform=platform, metascore=80, userscore=8.0)]
    return game
...
def test_platform_query_param_is_passed_through_to_the_use_case(client):
    ...
    use_case.list_games = AsyncMock(return_value=[_game(1, "Elden Ring", "PC")])

    response = test_client.get("/games", params={"platform": "PC"})

    assert response.status_code == 200
    use_case.list_games.assert_awaited_once_with(platform="PC", q=None, sort="rating")
