...
- **FR-005**: System MUST reset its "already processed today" tracking at the start of each new calendar day and MUST resume sourcing from "New Releases" first. If "New Releases" yields fewer than 20 not-yet-processed games for the day, System MUST top up the remainder within the same run from the "See All / Newest" listing (starting at the correct page), so each run processes 20 games whenever the combined sources have that many left for the day. [Revised during implementation: on every run *after* the day's first, System ALSO always rechecks "See All / Newest" page 1 as a second, fixed top-up source — not only when the primary source falls short. This is a drift correction: "See All"'s own pagination cursor only advances forward and never revisits page 1, but the live listing itself drifts as new games are continuously added to page 1 throughout the day, so without this recheck, a game published after the day's first run would be permanently missed once the cursor advanced past page 1. This recheck's own top-up is capped by the same 20-games-per-run ceiling as any other source and never advances the "See All" pagination cursor itself (domain/rules.py's `plan_ingest`/`advance_state`, research.md §12).]
